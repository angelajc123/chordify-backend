...
Deno.test("Principle: User creates, modifies, and views a chord progression", async () => {
  ...
  try {
    ...
    let progressionResult = await concept.getProgression({ progressionId });
    ...
    assertEquals(progression.chords.length, 0);
    ...
    progressionResult = await concept.getProgression({ progressionId });
    assertNotEquals("error" in progressionResult, true);
    progression = (progressionResult as { progression: any }).progression;
    assertEquals(progression.chords.length, 3);
    assertEquals(progression.chords[0].chord, null);
    assertEquals(progression.chords[1].chord, null);
    assertEquals(progression.chords[2].chord, null);
    ...
    assertNotEquals("error" in progressionResult, true);
    progression = (progressionResult as { progression: any }).progression;
    assertEquals(progression.chords[0].chord, "Cmaj");
    assertEquals(progression.chords[1].chord, null);
    assertEquals(progression.chords[2].chord, "G7");
    ...
    assertNotEquals("error" in progressionResult, true);
    progression = (progressionResult as { progression: any }).progression;
    assertEquals(progression.chords[0].chord, null); // Original pos 1 is now 0
    assertEquals(progression.chords[1].chord, "Cmaj"); // Original pos 0 is now 1
    assertEquals(progression.chords[2].chord, "G7"); // Remains at pos 2
    ...
    assertNotEquals("error" in progressionResult, true);
    progression = (progressionResult as { progression: any }).progression;
    assertEquals(progression.chords.length, 2);
    assertEquals(progression.chords[0].chord, null); // Was pos 1, now 0
    assertEquals(progression.chords[1].chord, "G7"); // Was pos 2, now 1
    ...
    const listResult = await concept.listProgressions();
    ...
    const deletedProgression = await concept.getProgression({ progressionId });
...
Deno.test("Action: createProgression successfully creates a new progression", async () => {
  ...
  try {
    ...
    const retrieved = await concept.getProgression({ progressionId: id });
    ...
    assertEquals(retrievedProg.chords.length, 0);
  } finally {
    await client.close();
  }
});
...
Deno.test("Action: addSlot appends a null slot and requires valid progression ID", async () => {
  ...
  try {
    ...
    let progResult = await concept.getProgression({ progressionId: pId });
    assertNotEquals("error" in progResult, true);
    let prog = (progResult as { progression: any }).progression;
    assertEquals(prog.chords.length, 1);
    assertEquals(prog.chords[0].chord, null);

    await concept.addSlot({ progressionId: pId });
    progResult = await concept.getProgression({ progressionId: pId });
    assertNotEquals("error" in progResult, true);
    prog = (progResult as { progression: any }).progression;
    assertEquals(prog.chords.length, 2);
    assertEquals(prog.chords[1].chord, null);
...
Deno.test("Action: setChord updates a slot and requires valid progression ID and position", async () => {
  ...
  try {
    ...
    assertNotEquals("error" in progResult, true);
    let prog = (progResult as { progression: any }).progression;
    assertEquals(prog.chords[0].chord, "Am");
    assertEquals(prog.chords[1].chord, null);
...
Deno.test("Action: deleteChord sets chord to null and requires valid progression ID and position", async () => {
  ...
  try {
    ...
    const progResult = await concept.getProgression({ progressionId: pId });
    assertNotEquals("error" in progResult, true);
...
Deno.test("Action: deleteSlot removes a slot and requires valid progression ID and position", async () => {
  ...
  try {
    ...
    let prog = (progResult as { progression: any }).progression;
    assertEquals(prog.chords.length, 1);
    assertEquals(prog.chords[0].chord, "G"); // 'G' should now be at position 0
...
Deno.test("Action: reorderSlots reorders slots and requires valid progression ID and positions", async () => {
  ...
  try {
    ...
    assertNotEquals("error" in progResult, true);
    let prog = (progResult as { progression: any }).progression;
    assertEquals(prog.chords.map((s: any) => s.chord), ["G", "Am", "C"]);
    ...
    assertNotEquals("error" in progResult, true);
    prog = (progResult as { progression: any }).progression;
    assertEquals(prog.chords.map((s: any) => s.chord), ["Am", "G", "C"]);
...
Deno.test("Action: deleteProgression removes a progression and requires valid ID", async () => {
  ...
  try {
    ...
    const prog1Result = await concept.getProgression({ progressionId: pId1 });
    assertEquals("error" in prog1Result, true, "Progression 1 should be deleted.");

    const prog2Result = await concept.getProgression({ progressionId: pId2 });
...
Deno.test("Query: getProgression retrieves a progression or returns error for invalid ID", async () => {
  const [db, client] = await testDb();
  const concept = new ProgressionBuilderConcept(db);
  ...
  try {
    ...
    // Valid case
    const result = await concept.getProgression({ progressionId: pId });
    ...
    const errorResult = await concept.getProgression({
      progressionId: invalidId,
    });
    assertEquals("error" in errorResult, true);
    assertEquals(
...
Deno.test("Query: listProgressions returns all progression identifiers and names in reverse chronological order", async () => {
  ...
  try {
    // No progressions
    let listResult = await concept.listProgressions();
